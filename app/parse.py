...
def create_single_course(course_soup, time):
    return Course(
        ...
        course_type=CourseType.FULL_TIME if CourseType.FULL_TIME.value == time
...
def get_all_courses() -> [Course]:
    ...
    courses = soup.select_one("#all-courses").select(
    ...
    for course_group in courses:
        ...
        for course in course_group.select(".CourseCard_cardContainer__7_4lK"):
            courses_list.append(create_single_course(course, time))
